...
if (!OWNER_EMAIL || !EMAIL_USER) return res.status(500).json({ ok:false, error:"Server email is not configured." });
...
// Send uploaded files to the owner email
...
// Optional: Send receipt to the client
...
// Delete uploaded files from server after sending
for (const f of req.files) fs.unlink(f.path, () => {});
res.json({ ok:true });
} catch (err) {
console.error("UPLOAD_ERR:", err?.message || err);
if (err?.code === "LIMIT_FILE_SIZE") return res.status(413).json({ ok:false, error:"Each file must be ≤ 20MB." });
if (err?.code === "LIMIT_FILE_COUNT") return res.status(413).json({ ok:false, error:"Max 10 files per upload." });
